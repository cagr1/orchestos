...
- [ ] I.7 🧠 **Graph runner muestra filas `done` sin propósito** — 5 filas ya completadas: ¿qué acción tiene sentido ahí? ¿Qué hace "Run full plan" cuando todo está done (debería decir "nada que correr" en vez de un botón activo)? Definir el estado vacío/completado de esta pantalla: si no hay pendientes, comunicarlo claramente y ofrecer lo único útil (ej. reset de statuses para re-correr, o link a crear tareas).
- [ ] I.8 🧠 **Opciones de borrado en todas las tabs** — hoy solo Tasks y Skills tienen DELETE. Runs, Memory (entries), Specs (solo archive), Instincts (solo reject) no tienen forma de eliminar registros viejos desde el dashboard. Definir por tab: qué se puede borrar, con confirm(), endpoint + botón. Relacionado con la pregunta de producto: ¿los registros `done`/viejos son relevantes para el usuario o solo ruido?
- [ ] I.9 ⚡ **Specs sin i18n** — botones "Aprobar"/"Archivar" hardcodeados en español (`screens-ops.js:1292-1294`) y headers de tabla en inglés hardcodeado (`:1349`), mientras el idioma default es inglés. Mover todo a `i18n.js` (en/es) como el resto del dashboard.
