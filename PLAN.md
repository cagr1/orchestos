...
- [X] F3 (⚡) Añadir entradas al diccionario i18n para el modo avanzado y los ítems que aparecen/desaparecen. Revisar que todas las pantallas de operador (Runs, Memory, Specs) tienen su traducción completa.
...
- [x] F5 (⚡) Pantalla de inicio adaptativa ✓ 2026-06-04 — `fetchAll()` extendido: si hay `attentionCount > 0 || cost > threshold` → settings (Control Center, ambos modos). Si no hay atención y `orchestos-mode = advanced` → runs. Si no hay atención y modo normal → tasks (default). 369 tests · 0 fail.
