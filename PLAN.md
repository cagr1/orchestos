...
- [ ] A2 Implementar `runToolLoop()` en `tool-call.ts` + `FETCH_URL_TOOL` + wiring en `handleApiChat` (⚡ siguiendo el diseño de A1) — sin guard SSRF todavía (solo `fetch()` directo), para testear el loop multi-turno aislado con mocks. Fallback al single-turn actual sin cambios para Ollama y modelos sin tool-calling.
