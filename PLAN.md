...
- [ ] I.6 🧠 **Memory cards solo muestran título** — al hacer clic en una memoria solo se ve el `topicKey`, sin contenido útil. El concepto dice "file paths, conventions, past decisions" pero la card no comunica NADA de eso. Investigar: ¿el `content` real en `memory_entries` es sustancial y el problema es solo de UI (preview truncado / expand roto), o el contenido guardado es realmente pobre? Si es UI: expandir debe mostrar contenido completo + metadata (scope, fechas, origen). Si el contenido es pobre: es un problema del motor de memoria, documentar por separado.
