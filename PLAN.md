...
- [ ] A.1 🧠 Doc de diseño (`docs/chat-task-detection-design.md`) que decida explícitamente: (a) forma exacta del LLM call clasificador (prompt, modelo barato, formato de salida) y su costo por mensaje; (b) qué tools de lectura exponer (`PLAN.md`/`tasks.yaml`/`IDEAS.md`) y su `ToolDef` sobre `runToolLoop()`; (c) el punto de control humano exacto — cómo se ve "sugerir la conversión" en la UI sin cruzar a auto-run; (d) el criterio de evidencia que gatea reemplazar la heurística de 3+ mensajes (no implementar sin datos reales de falsos negativos).
- [ ] A.2 🔍 Revisión del doc con Carlos antes de abrir B.

### Bloque B — Implementación (pendiente de diseño de A.1)
- [ ] B.1 🧠 Clasificador semántico de intención de tarea.
- [ ] B.2 ⚡ Tools de lectura (`PLAN.md`/`tasks.yaml`/`IDEAS.md`) registradas en `runToolLoop()`.

### Bloque C — Superficie
- [ ] C.1 ⚡ UI de sugerencia (no auto-run) cuando el clasificador detecta intención de tarea.
