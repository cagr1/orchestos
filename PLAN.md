...
- [x] **FRONT.1** (🧠, 2026-06-29) `app.js`: `modelSupportsReasoning(modelId, orModels)` — true solo si `state.orModels` (datos reales de `/api/chat/models`, BACK.4) tiene esa entrada con `supportsReasoning:true`, nunca asumido. `state.chatEffort` default `'medium'` (en memoria, sin persistencia — eso es FRONT.2). `screens-core.js` (`SCREENS.chat`): `<select id="chat-effort-select">` (low/medium/high) renderizado solo cuando `modelSupportsReasoning()` es true para el modelo elegido; `change` de `#chat-model-select` ya dispara `App.rerender()` así que el selector aparece/desaparece solo al cambiar de modelo. `send()` agrega `effort` al body de `POST /api/chat` únicamente cuando el control está visible (mismo guard). Labels vía `t('chat.effort.*')` — claves nuevas, `t()` cae al nombre de la key si no existen todavía (FRONT.3 las completa). **Verificado en vivo con Playwright** contra el dashboard real: seleccionar `deepseek/deepseek-r1` muestra el selector con las 3 opciones (`low`/`medium`/`high`); cambiar a `openai/gpt-4o-mini` lo oculta. `node --check` limpio en ambos archivos.
- [x] **FRONT.2** (⚡, 2026-06-29) Persistir la selección de esfuerzo en `localStorage` (mismo patrón que el estado del sidebar), default `'medium'` cuando el modelo lo soporta. `state.chatEffort` lee de `localStorage.getItem('orchestos-chat-effort')` en `app.js:58`; el handler `change` de `#chat-effort-select` guarda vía `localStorage.setItem()` en `screens-core.js:199`. El envío de `effort` en el body de `POST /api/chat` ya lo hacía FRONT.1 condicionado a `modelSupportsReasoning()` — intacto.
- [x] **FRONT.3** (⚡, 2026-06-29) i18n en/es: claves `chat.effort.{label,low,medium,high}` agregadas en ambos bloques de `i18n.js`. Inglés: "Reasoning effort" · "Low" · "Medium" · "High". Español: "Esfuerzo de razonamiento" · "Bajo" · "Medio" · "Alto". Las claves ya eran consumidas por `screens-core.js` desde FRONT.1 sin existir — ahora están definidas.
- [x] **FRONT.4** (🔍 Claude, 2026-06-29) Verificado en vivo con Playwright contra el dashboard real (headless, modelos mockeados con `supportsReasoning` real): 7/7 tests pasan — (1) effort oculto con modelo sin reasoning, (2) visible con deepseek-r1 con default 'medium', (3) cambio a 'high' persiste en localStorage, (4) recarga restaura 'high' desde localStorage, (5) cambiar a gpt-4o-mini oculta el control, (6) POST /api/chat incluye `effort:'high'` con modelo reasoning, (7) POST /api/chat omite `effort` con modelo sin reasoning. `node --check` limpio en los 3 archivos modificados.
- [x] **FRONT.5** (🧠, 2026-06-29) Bug encontrado por Carlos en revisión posterior a FRONT.4: el selector de esfuerzo se solapaba visualmente con el botón de refresh de modelos. Causa raíz (`screens.css`): `.model-sel` tiene `flex:1` sin `min-width:0` — el `<select>` mantiene su ancho intrínseco (texto largo de las opciones, ej. "DeepSeek: R1 — $0.70/M") y desborda el wrap de 260px (`.model-select-wrap`), empujando el botón `[data-refresh-models]` fuera de su sitio hacia donde se renderiza `#chat-effort-select`. Fix: agregado `min-width:0` (deja que flexbox encoja el select de verdad) + `text-overflow:ellipsis; overflow:hidden; white-space:nowrap` (evita que el texto largo se desborde visualmente una vez encogido). **Verificado en vivo con Playwright**: antes del fix, `refreshBox.right` (1122.5) excedía `wrapBox.right` (1085.5) y se solapaba con `effortBox` (1093.5–1174); después del fix, `refreshBox.right === wrapBox.right` (1085.5) y `overlap === false`. Captura confirma "Medium" separado del botón de refresh. Hallazgos preexistentes de `impeccable` (side-tab borders, L323/345/367/375) son anteriores a este cambio y ya están documentados en la auditoría visual de este archivo — fuera de alcance acá.
- [x] **FRONT.6** (🧠, 2026-06-29) Rediseño pedido por Carlos tras FRONT.5: orden modelo→esfuerzo→refresh (en vez de mezclado), refresh automático en vez de botón manual, estilo de búsqueda integrada tipo Hermes (un solo control de altura única, no dos filas apiladas), y flecha de `<select>` cambiada de triángulo relleno a chevron simple. Implementado como un combobox custom (`buildModelCombo()`/`buildComboOptions()`/`modelLabelFor()` en `app.js`) que reemplaza por completo el viejo layout de chat (input de búsqueda arriba + `<select>` nativo + botón refresh) — trigger de altura única (label + chevron `ICON.chev`, ya existente) que abre un panel absolutamente posicionado con buscador integrado arriba y lista filtrable abajo; sin botón de refresh visible — `loadOrModels()` (ya gateado por TTL internamente) se llama solo al abrir el panel, silenciosamente. Estado del panel (`st.chatModelComboOpen`) vive en `state` (sobrevive a los rerenders); cierre por click-afuera implementado con UN listener global registrado una sola vez en `boot()` (no dentro de `wire()`, que se re-ejecuta en cada rerender y duplicaría el listener). `buildModelSelect()` (usado por el modal de borrador de tareas, sin tocar) perdió la rama `withSearch` ahora muerta. Chevron de `.model-sel` (afecta también el selector de esfuerzo, que sigue siendo `<select>` nativo) cambiado de triángulo relleno a trazo chevron simple — consistente en todo el proyecto donde se use esa clase. CSS muerto eliminado: `.chat-model-sel`, `.model-select-wrap`, `.model-search`. **Verificado en vivo con Playwright** contra el dashboard real: trigger de altura única con label+chevron · abrir el panel muestra buscador + lista (sin segunda fila apilada) · escribir "deepseek" filtra en vivo sin perder foco · click en una opción cierra el panel, actualiza el modelo y el selector de esfuerzo aparece/desaparece según corresponda · click afuera del combo cierra el panel · 0 errores de consola. `node --check` limpio en `app.js`/`screens-core.js`.
- [x] **FRONT.7** (🧠, 2026-06-29) Bug real reportado por Carlos, no relacionado con FRONT.6: escribir en el chat o en la búsqueda de modelos y dejar pasar tiempo borraba el texto solo. Causa raíz, **preexistente** (no introducida por FRONT.1-6): `App.fetchAll()` ([app.js:251](src/dashboard/public/app.js:251)) corre cada 30s vía `setInterval` (`boot()`) sin importar la pantalla activa, y terminaba con `this.rerender()` **incondicional** — `rerender()` reemplaza el `innerHTML` completo de `#main`, borrando silenciosamente cualquier input no controlado por `state` (textarea del chat, buscador del combo de modelo, cualquier filtro) en el que el usuario estuviera escribiendo en ese instante. Confirmado con un test en vivo que esperó los 32s reales: sin el fix, tanto el textarea como el buscador del combo quedaban vacíos. Fix de raíz: si `document.activeElement` es un `INPUT`/`TEXTAREA` dentro de `#main`, el poll de fondo ya no hace el rerender destructivo — solo refresca el badge de Skills vía `syncNav()` (lo único que de hecho depende de ese fetch para el chrome persistente); el próximo poll (30s después, cuando el usuario ya no esté escribiendo) sí refresca con normalidad, igual que cualquier pantalla con datos en vivo (Tasks/Runs) que no tenga foco en un input al momento del poll. Complementario: `fetchInstincts()` ([app.js:100](src/dashboard/public/app.js:100)) también llamaba `this.rerender()` sin condición — ningún badge de nav depende de instincts, así que ahora solo rerenderiza si `state.screen === 'instincts'` (defensa en profundidad, por si se llama fuera de `fetchAll()` en el futuro). **Verificado en vivo con Playwright, con el tiempo real (no simulado)**: escribí en el textarea del chat y en el buscador del combo, esperé los 32s reales del intervalo, y ambos valores sobrevivieron intactos (`"texto que no se debe borrar otra vez"` / `"deepseek"`), con el foco todavía en el control correspondiente. 0 errores de consola. `node --check` limpio.
