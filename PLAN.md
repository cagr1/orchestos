...
- [ ] G.1 🧠 **[P1] Overflow horizontal en mobile** — confirmado en vivo a 375px (Project → Agent guide): texto y header cortados, sin ningún breakpoint responsive en `styles.css`/`screens.css`. Fix: breakpoint real `<768px`, sidebar colapsable/icon-only forzado, contenedor principal con `max-width:100%`.
- [ ] G.2 🧠 **[P1] Filas de tabla sin acceso completo por teclado** — Tasks (`screens-core.js:575`) no tiene `tabindex` (Runs/Specs sí, `screens-ops.js:532,1307`); ninguna de las 3 tiene `keydown` (Enter/Space) pese a que el sidebar sí lo implementa (`app.js:1575-1578`, mismo patrón a reusar). Fix: `tabindex="0"` en la fila de Tasks + un handler de teclado compartido para las 3 tablas.
- [ ] G.3 ⚡ **[P2] Side-stripe border sistemático** — `.proj-helper` (`screens.css:351`, `border-left: 3px solid var(--accent)`) es el ban explícito de Impeccable ("side-stripe borders... nunca intencional"), reusado en el panel de conflictos de memoria. Fix: fondo tintado (`--surface-2`) + ícono, sin borde de acento lateral.
- [ ] G.4 ⚡ **[P2] Transiciones sin `prefers-reduced-motion`** — 33 transiciones/animaciones en `styles.css`+`screens.css`, solo 1 regla `prefers-reduced-motion` — `PRODUCT.md` lo declara requisito explícito. Fix: regla global `@media (prefers-reduced-motion: reduce)` que anule/acorte transiciones no esenciales.
- [ ] G.5 ⚡ **[P3] 2 colores hardcodeados fuera de tokens** — `screens-core.js:428` (`#1a1a2e`, fondo de bloque de error) y `screens-core.js:903` (`var(--warning, #e0a030)`, fallback que no coincide con `--warning` real). Fix: usar `var(--surface-2)`/`var(--warning)` sin fallback custom.
- [ ] G.6 🔍 Gate en vivo — reverificar los 5 fixes contra el dashboard real (Playwright: mobile 375px, Tab+Enter en las 3 tablas, screenshot del helper sin side-stripe, `prefers-reduced-motion` emulado).
