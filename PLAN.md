...
- [x] F3.1 ⚡ (2026-07-02) `harness.ts:260-265` (catch de `ctx.provider.chat()`) ahora llama `insertRun` con `status:'failed'`, `result: e.message`, `input_tokens/output_tokens/usd_cost: 0` (ningún LLM response fue devuelto), `snapshot_before: JSON.stringify(before)` (ya en scope, computado en `harness.ts:224`), `snapshot_after: null`, `qa_verdict: null`, `qa_reason: null`, `files_attempted/authorized/blocked: null` (no hubo parse), más los campos de telemetría estándar (`constitution_rules`, `context_source`, `context_tokens`, `embed_hits`, `context_warnings_json`) siguiendo el mismo patrón de los paths de parse error (~274) y contract violation (~286). El `runId` insertado NO se devuelve en `TaskResult` todavía — eso es F3.2 (cambio de contrato del `TaskResult`, separado de la evidencia). `bunx tsc --noEmit` limpio · `bun test` **538 pass · 0 fail** (43 archivos, mismo conteo que pre-F3.1 — F3.1 no añade tests, son F3.3) en 2 corridas (default + aleatorio `sort -R`).
- [x] F3.2 ⚡ (2026-07-02) Los 6 paths de fallo del harness ahora capturan el retorno de `insertRun` y lo devuelven en `TaskResult.runId` en vez de descartarlo: F3.1 LLM call catch (`harness.ts:263-264`), parse error (`harness.ts:276-277`), contract violation (`harness.ts:288-289`), missing outputs (`harness.ts:311-320`), check fail (`harness.ts:365-380`) y QA fail (`harness.ts:408-415` — comparte el `runId` entre los dos returns `failed_permanent` y `retry`). Cambios mecánicos: `insertRun({...})` → `const runId = insertRun({...})` y `runId: ''` → `runId` en el return del mismo scope (cada uno en su propio block, sin shadowing del `runId` del path de éxito en `harness.ts:426`). Quedan 3 returns con `runId: ''` intencionalmente: dry-run (línea 220), context budget pre-flight (línea 252) y catch-all inesperado (línea 447) — ninguno llama `insertRun`, no hay nada que capturar (consistente con el plan: F3.2 aplica a los 6 paths que `insertRun` descarta). `bunx tsc --noEmit` limpio · `bun test` **538 pass · 0 fail** (43 archivos) en 2 corridas (default + aleatorio `sort -R`).
- [x] F3.3 ⚡ (2026-07-02) `src/__tests__/harness-evidence.test.ts` (nuevo, 6 tests) cubre los 6 paths de fallo de F3.1+F3.2 con un mock provider basado en `globalThis.fetch` (mismo patrón que `harness-retry.test.ts` y `provider-max-tokens.test.ts` — cada `chat()` hace una sola llamada, sin ventana de carrera). Para cada path se afirma que `TaskResult.runId` es no-vacío Y que `getRun(runId)` devuelve una fila con `task_id` correcto y el mensaje esperado (busca en `row.result` ∪ `row.qa_reason` porque missing/check/QA guardan el motivo en `qa_reason` y un summary tipo `"check fail — reverted N file(s)"` en `result`, mientras que F3.1/parse/contract guardan el `e.message` directo en `result`). Los 6 tests: (1) F3.1 LLM call catch — fetch lanza `Error('test provider boom 401')` → fila con `'test provider boom 401'`, `result.status === 'failed'`; (2) parse error — fetch devuelve texto sin bloques `<<<FILE:...>>>` → fila con `'No <<<FILE:...>>>'`; (3) contract violation — fetch devuelve `<<<FILE:rogue.txt>>>` con `output: ['out.txt']` → fila con `'CONTRACT VIOLATION'`, `row.status === 'blocked'` (no `'failed'`, es la convención del path); (4) missing declared outputs — fetch devuelve solo `out-a.txt` con `output: ['out-a.txt','out-b.txt']` → fila con `'missing declared output(s): out-b.txt'`, `result.status` es `'failed'` o `'retry'` (depende de `retry_count vs MAX_RETRIES`); (5) check fail — fetch devuelve `out.txt` válido y `checks: [{cmd:'false'}]` → fila con `'check failed: false exit 1'`, `result.status` es `'failed'` o `'retry'`; (6) QA fail — fetch 1 devuelve `out.txt` válido, fetch 2 (QA judge) devuelve `{"verdict":"fail","reason":"qa fail test reason"}` → fila con `'QA fail'` y `row.qa_verdict === 'fail'`. `runMigrations()` en `beforeAll` (la DB real `~/.orchestos/db.sqlite` la necesita en CI fresco). `bunx tsc --noEmit` limpio · `bun test` **544 pass · 0 fail** (44 archivos, +6 vs. 538 pre-F3.3) en 2 corridas (default + aleatorio `sort -R`).
- [x] F3.4 🔍 (2026-07-02) Verificado en vivo: tarea desechable `f3-4-test-invalid-key` (backup de `tasks.yaml`, restaurado; fila de `runs` borrada al cerrar) corrida con `OPENROUTER_API_KEY` inválida inyectada solo vía env del proceso (`process.env` tiene prioridad sobre `~/.orchestos/.env` — el archivo real nunca se tocó, confirmado post-run). `runs --detail` mostró fila real: `status: failed`, `result: OpenRouter error 401: {"error":{"message":"Missing Authentication header","code":401}}`, `input/output: 0 tokens` (correcto — ningún LLM response fue devuelto), `runId` no vacío (existe en la DB). Confirma el path F3.1 con un fallo de proveedor real, no mock. `tsc` limpio · `bun test` 544 pass · 0 fail tras limpieza.

### Bloque F4 — Contrato con paths normalizados
- [x] F4.1 ⚡ (2026-07-02) Helper exportado en `src/run/contract.ts` (entre `parseLLMResponse` y `enforceContract`). Implementación: `p.replaceAll('\\', '/')` → `replace(/\/+/g, '/')` (colapsa cualquier run de `/`) → `while (s.startsWith('./')) s = s.slice(2)` (solo prefijo, no `./` interno — coincide con "prefijo `./` repetido" del plan) → `if (s.length > 1 && s.endsWith('/')) s = s.slice(0, -1)` (guard para no mutilar el root `/`). `..` NO se resuelve: `'../x'` → `'../x'`, `'src/../../x'` → `'src/../../x'`, `'a/b/../c'` → `'a/b/../c'` (la protección anti-escape se conserva). Verificación manual con 14 casos cubre todos los ejemplos del plan + edge cases (`'.'` → `''`, `'/'` → `'/'`, `'././src/a.ts'` → `'src/a.ts'`, `'src/a///'` → `'src/a'`). `bunx tsc --noEmit` limpio · `bun test` **544 pass · 0 fail** (44 archivos, mismo conteo que pre-F4.1 — F4.1 no añade tests, son F4.3). F4.2 y F4.3 consumen este helper.
- [x] F4.2 ⚡ (2026-07-02) `enforceContract` (`src/run/contract.ts:65-108`) ahora normaliza ambos lados: `allowedPaths.map(normalizeRelPath)` para los declarados, `normalizeRelPath(file.path)` para cada path emitido por el LLM. `attempted`/`authorized`/`blocked`/`written` guardan el path normalizado (evidencia normalizada, per "el path que se ESCRIBE y el que se guarda en evidencia es el normalizado"). El `join(root, normalizedPath)` en el write loop usa el path normalizado — `mkdirSync`/`writeFileSync` apuntan a la versión canónica. El mensaje de error de violación lista los `normalizedAllowed` (no los raw). En el harness, el cálculo de `missingOutputs` (`harness.ts:301`) ahora es `ctx.task.output.map(normalizeRelPath).filter(p => !contractResult.written.some(f => f.path === p))` — el lado izquierdo (declarados) se normaliza para comparar contra el lado derecho (`contractResult.written` ya viene normalizado de `enforceContract` post-F4.2). `snapshotHashes`/`snapshotContents` NO se tocan — siguen recibiendo `ctx.task.output` crudo, per "las claves del snapshot son los paths del contrato" del plan. Verificación manual con 5 casos: `./src/a.ts`→`src/a.ts` (escrito y evidenciado normalizado), `src\a.ts`→`src/a.ts`, `src//a.ts`→`src/a.ts`, `../escape.txt`→throw con `Allowed: src/a.ts` (anti-escape conservado), contract `./src/a.ts` matchea LLM `src/a.ts` (ambos lados normalizan al mismo). `bunx tsc --noEmit` limpio (necesitó cambiar el segundo loop a `for...of` por `noUncheckedIndexedAccess` en `response.files[i]`) · `bun test` **544 pass · 0 fail** (44 archivos, mismo conteo que pre-F4.2 — F4.2 no añade tests, son F4.3) en 2 corridas (default + aleatorio `sort -R`).
- [x] F4.3 ⚡ (2026-07-02) 8 tests nuevos en `src/__tests__/contract.test.ts` (nuevo describe `enforceContract — path normalization (F4.3)`) + 1 test nuevo en `src/__tests__/harness-evidence.test.ts` (F4.3). Cobertura: (1) `./src/a.ts` autoriza contra `src/a.ts` (helper `expectAuthorizedAt` verifica que el archivo en disco está en el path normalizado, no en `./src/a.ts`); (2) `src\a.ts` autoriza (separador Windows); (3) `src//a.ts` autoriza (colapsado); (4) `././/src/a.ts` combina los 3 (prefijo + separador + colapso); (5) `../x` sigue bloqueado; (6) `src/../../x` sigue bloqueado (anti-escape conservado); (7) `./../x` sigue bloqueado (incluso con `./` stripped primero); (8) contract con `./src/a.ts` matchea LLM `src/a.ts` (ambos lados normalizan al mismo). El test de harness usa mock provider que emite `<<<FILE:./out-a.txt>>>` con `output: ['out-a.txt', 'out-b.txt']` → la fila en `runs` tiene `'missing declared output(s): out-b.txt'` (NO `'out-a.txt, out-b.txt'` que sería el falso positivo), `files_authorized` contiene `'out-a.txt'` (normalizado) y NO contiene `'./out-a.txt'`, `files_blocked` NO contiene `'out-a.txt'`. `bunx tsc --noEmit` limpio · `bun test` **553 pass · 0 fail** (44 archivos, +9 vs. 544 pre-F4.3) en 2 corridas (default + aleatorio `sort -R`).
- [x] F4.4 🔍 (2026-07-02) Verificado en vivo: tarea desechable `f4-4-test-dotslash-path` (backup de `tasks.yaml`, restaurado; fila de `runs` borrada al cerrar) con prompt que instruye explícitamente al modelo a emitir `<<<FILE:./src/f4-test-output.txt>>>` (con el prefijo `./`) — corrida real vía OpenRouter. `runs --detail` confirmó: `attempted`/`written: src/f4-test-output.txt` (normalizado, sin `./`), `blocked: (none)` — sin `CONTRACT VIOLATION`. El QA falló por un detalle de formato ortogonal (mismo patrón que F1.4/F3.4), no afecta lo que F4.4 verifica: el contrato no generó falso positivo pese al `./` emitido por el LLM. `tsc` limpio · `bun test` 553 pass · 0 fail tras limpieza.
