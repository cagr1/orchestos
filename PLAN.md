...
- [ ] I.1 ⚡ **Settings→Model routing: fila "File" innecesaria + líneas contaminantes** — el path completo de `orchestos.config.yaml` no aporta (el badge "found" ya lo dice); hay 2 líneas divisorias debajo que ensucian. Quitar la fila File (o moverla a un tooltip del badge) y las líneas sobrantes.
- [ ] I.2 🧠 **Settings→Model routing: roles sin respiración + dropdown QA clippeado** — los 5 roles están pegados verticalmente sin ritmo, no usan el espacio horizontal disponible (podrían ser grid de 2 columnas); y el panel del combo de QA (último rol) queda atrapado dentro del card — el dropdown se corta y no se puede ver hacia abajo (mismo problema de `overflow` + `position:absolute` que Impeccable documenta: usar `position:fixed`/popover o dejar overflow visible en el card).
- [ ] I.3 🧠 **Líneas divisorias por todo el dashboard** — patrón sistémico: títulos y secciones separados con bordes/`<hr>`/`border-bottom` en vez de espaciado y jerarquía tipográfica. Carlos lo señala como el síntoma #1 de diseño no-premium ("no quiero ver líneas a manera de separación, eso está en todo el dashboard"). Requiere pasada exhaustiva: reemplazar separación-por-línea por separación-por-espacio (margin/padding escalado) y jerarquía real (peso/tamaño/color de texto), dejando líneas solo donde delimitan regiones funcionales reales (ej. borde de tabla).
- [ ] I.4 ⚡ **Settings→Project: botón "Reset test data" pegado al card** — sin margen/separación respecto al contenido del card. Dar aire y jerarquía visual de acción destructiva.
- [ ] I.5 🧠 **Memory conflicts sin acción** — el panel muestra 5 conflictos sin resolver pero no hay NADA que hacer con ellos (ni resolver, ni borrar). Mostrar información inaccionable es peor que no mostrarla. Falta: acción de resolver/descartar por conflicto (endpoint + botón), o al menos borrar. Revisar qué expone `memory.ts` (`listConflicts()` existe; verificar si existe `resolveConflict`/delete en el módulo de DB o hay que crearlo).
