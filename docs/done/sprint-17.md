...
Origen: IDEAS.md #15, decisión de Carlos (2026-07-02) tras la revisión estratégica que originó el Sprint 16 — "la jugada que convierte a OrchestOS de runner casero a capa de confianza". Tesis: el valor diferenciador de OrchestOS es la capa de verificación (contrato + checks + evidencia + QA + diagnose), no el ejecutor propio — un ejecutor externo (Claude Code headless) es una tercera implementación de `ExecutorEngine` (Sprint 16 G) que **edita** en vez de reescribir, eliminando por diseño el riesgo de fidelidad que G.5 encontró en el agéntico interno con archivos grandes.
...
**Decisiones de diseño Sprint 17**
...
- Un gate 🔍 con dinero real volvió a encontrar un bug de infraestructura real que ningún test con mocks había mostrado (mismo patrón que G.5, Sprint 14 D2/D3, Sprint 13 Bloque A — ver [[feedback-verificar-gates-en-vivo]]) — el parseo de `git status --porcelain` corrompido por un `.trim()` compartido demasiado agresivo.

**Métrica Sprint 17 — SÍ (2026-07-05)**
