...
|---|---|---|
| F0 | Integridad — suite determinista, `tasks.yaml` reconciliado, providers arreglados (prerequisito de Sprint 15→16, cerrado la mañana del 2026-07-02) | ✅ SÍ |
...
- **G.3 (agéntico)**: `executors/agentic.ts` reusa `runToolLoop()` (Sprint 13) tal cual. Selección: `Task.engine` gana sobre `orcheConfig.executorEngine`, default absoluto `single-shot`. Fallback automático a single-shot si el modelo no soporta tool-calling, con aviso en el log.
...
**Decisiones de diseño Sprint 16**
...
- Un gate 🔍 con dinero real encontró 2 bugs de infraestructura que ningún test con mocks había mostrado (`max_tokens` hardcodeado en 4 lugares + presupuesto sin clamp contra el tope real del proveedor) — mismo patrón que Sprint 14 D2/D3 y Sprint 13 Bloque A: verificar en vivo contra el sistema real, no solo contra mocks (ver [[feedback-verificar-gates-en-vivo]]).
...
**Lista prohibida Sprint 16** _(lo que NO se hizo — referencia histórica)_
...
**Métrica Sprint 16 — SÍ (2026-07-02)**
