...
- B1 (🧠) Decisión de arquitectura en `docs/done/design/autoskills-registry-design.md`: consumir el índice real de `cdn.jsdelivr.net/npm/autoskills/skills-registry/index.json` + `raw.githubusercontent.com` para contenido — `normalizeImport()` (Sprint 11) ya es agnóstica al formato de entrada, se reusa sin tocar — 2026-06-23
...
**Decisiones de diseño Sprint 13**
...
- Los gates 🔍 deben correr contra el sistema real, no solo `bun test` — los 3 bugs de Sprint 13 (SSRF false-positive, arity de `executeFetchUrl`, `idleTimeout`) solo aparecieron verificando en vivo; los mocks de los tests ya tenían la forma correcta y los escondían.
...
**Lista prohibida Sprint 13** _(lo que NO se hizo — referencia histórica)_
...
- Runner de grafo autónomo — eje de autonomía interna, distinto del eje de conexión externa de este mes. Candidato Sprint 14.
- `description` vacía en `GET /api/skills/registry` — `fetchRegistryList()` no la extrae del índice ni del frontmatter (requeriría N+1 fetches). Candidato Sprint 14 si genera fricción real.
...
**Métrica Sprint 13 — SÍ (2026-06-23)**
