...
const badges = await page.evaluate(() =>
  [...document.querySelectorAll('.tbl tbody tr.row .badge')].map(
    (b) => b.className + '|' + b.textContent.trim(),
  ),
)
log(
  badges.some((b) => b.includes('amber') && b.includes('draft')) &&
...
log(
  specsCell &&
    vanillaCell &&
    specsCell.pad === vanillaCell.pad &&
    specsCell.font === vanillaCell.font &&
...
log(
  specsHead &&
    vanillaHead &&
    specsHead.pad === vanillaHead.pad &&
    specsHead.font === vanillaHead.font,
...
log(
  (await page.locator('.tbl tbody tr.row').count()) === 3,
