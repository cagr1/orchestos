...
try {
  ...
  log(await page.locator('.header').evaluate((node) => node.getBoundingClientRect().height === 44), 'header mide 44px')
  ...
  log(await page.locator('#rightpanel').evaluate((node) => node.getBoundingClientRect().width > 0), 'rpToggle abre el inspector')
  await page.locator('#rpToggle').click()
