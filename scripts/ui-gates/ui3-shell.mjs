...
const rightEdgeGap = async () => page.evaluate(() => {
...
log(gapCollapsed !== null && Math.abs(gapCollapsed - gapExpanded) <= 1,
...
log(gapBeforeSidebar === gapAfterSidebar,
...
const measureTopRows = () => page.evaluate(() => {
  ...
  return { rootHeight, sidebar: read('.sidebar-toprow'), right: read('.rp-toprow'), header: read('.header') }
})
const checkTopRows = async (state) => {
  ...
  const noVerticalPadding = [rows.sidebar, rows.right].every(row =>
    row?.paddingTop === '0px' && row.paddingBottom === '0px')
  log(sameHeight && noVerticalPadding,
...
log(await page.locator('.app').getAttribute('data-sidebar') === 'expanded',
...
const tooltip = await page.locator('.nav-icon[data-nav]').first().evaluate(el => {
  const pseudo = getComputedStyle(el, '::after')
  const host = el.getBoundingClientRect()
  const px = value => Number.parseFloat(value) || 0
  const width = px(pseudo.width) + px(pseudo.paddingLeft) + px(pseudo.paddingRight) + px(pseudo.borderLeftWidth) + px(pseudo.borderRightWidth)
  const height = px(pseudo.height) + px(pseudo.paddingTop) + px(pseudo.paddingBottom) + px(pseudo.borderTopWidth) + px(pseudo.borderBottomWidth)
  ...
  while (parent && !parent.classList.contains('app')) {
    ...
    if ([cs.overflow, cs.overflowX, cs.overflowY].some(value => ['hidden', 'auto', 'scroll'].includes(value))) {
      clippingAncestors.push(parent.id || parent.className || parent.tagName)
    }
    parent = parent.parentElement
  }
  return {
    content: pseudo.content, display: pseudo.display, left, top, right: left + width, bottom: top + height,
    clippingAncestors, viewportWidth: window.innerWidth, viewportHeight: window.innerHeight,
  }
})
log(tooltip.content !== 'none' && tooltip.display !== 'none' && tooltip.clippingAncestors.length === 0,
  `regla 3: tooltip sin ancestros overflow hidden/auto/scroll (${tooltip.clippingAncestors.join(' → ') || 'ninguno'})`)
log(tooltip.left >= 0 && tooltip.top >= 0 && tooltip.right <= tooltip.viewportWidth && tooltip.bottom <= tooltip.viewportHeight,
...
const beforeSwap = await topRow.evaluate(row => {
  const read = selector => {
    ...
    const cs = getComputedStyle(el)
    return { display: cs.display, opacity: cs.opacity, visibility: cs.visibility, hasRect: el.getClientRects().length > 0 }
  }
  return { logo: read('.sidebar-toprow-logo'), collapse: read('#navCollapseBtn'), html: row.outerHTML }
})
await topRow.hover()
...
const visible = value => value.hasRect && value.display !== 'none' && value.opacity !== '0' && value.visibility !== 'hidden'
log(visible(beforeSwap.logo) && !visible(beforeSwap.collapse) &&
...
log(beforeSwap.html === afterSwap.html, 'regla 4: el hover-swap es CSS puro, el outerHTML de la fila no cambia')
...
log(screenAfterNav === 'skills', `criterio 5: click en navegación cambia window.state.screen a ${screenAfterNav}`)
const activeNav = await page.locator('.nav-icon.active').evaluateAll(items => items.map(item => item.dataset.nav))
log(activeNav.length === 1 && activeNav[0] === screenAfterNav,
...
log(/^\d+$/.test(badgeText), `criterio 7: el badge de skills muestra un número (${badgeText || 'vacío'})`)
const status = await page.locator('#statusBadge').evaluate(el => ({ state: el.dataset.state, text: el.textContent.trim() }))
log(['idle', 'running'].includes(status.state) && status.text === status.state.toUpperCase(),
...
log(await page.locator('#rpToprow').locator('#rpToggle').count() === 1 &&
...
log(rpOpen === 'expanded' && rpClosed === 'collapsed',
...
log(shellCounts.sidebar === 1 && shellCounts.header === 1 && shellCounts.rightpanel === 1,
...
log(errors.length === 0, `criterio 11: sin errores de consola en toda la corrida (${errors.join(' | ') || 'ninguno'})`)
