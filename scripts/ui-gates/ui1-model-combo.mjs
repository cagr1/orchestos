...
await page.evaluate(() => { window.state.settingsSection = 'routing'; window.App.rerender() })
await page.waitForTimeout(1200)
...
const planner = page.locator('[data-island="model-combo"]').filter({ has: page.locator('#role-planner') })
...
log(filtered > 0 && filtered < totalItems, `la búsqueda filtra de verdad (${totalItems} → ${filtered})`)
// Regla del combo vanilla: matchea por id O por nombre visible.
const byId = await page.evaluate(() =>
  [...document.querySelectorAll('[cmdk-item]')].every(i => i.getAttribute('data-value')?.toLowerCase().includes('claude')))
...
const willPick = await page.evaluate(() =>
  document.querySelector('[cmdk-item][data-selected="true"]')?.getAttribute('data-value')?.split(' ')[0] ?? null)
await page.keyboard.press('Enter')
await page.waitForTimeout(400)
const picked = await page.evaluate(() => document.getElementById('role-planner')?.value ?? null)
log(picked === willPick, `Enter elige el ítem resaltado y lo escribe en el hidden input (${picked})`)
...
log(label.length > 0 && !label.includes('undefined'), `el trigger muestra el label del modelo elegido ("${label}")`)
const readable = await page.evaluate(() =>
  ['role-planner','role-executor_heavy','role-executor_light','role-default','role-qa']
...
const afterRerender = await page.evaluate(() => document.getElementById('role-planner')?.value ?? null)
log(afterRerender === beforeRerender && !!afterRerender,
...
const anchored = async () => await page.evaluate(() => {
...
await page.evaluate(() => { document.querySelector('.app').style.setProperty('--sidebar-w-exp', '280px') })
await page.waitForTimeout(250)
const a2 = await anchored()
log(a1 && a2 && a1.dx === a2.dx && a2.below,
  `regla 1 (anclaje al trigger, no al DOM): dx estable ${a1?.dx} → ${a2?.dx}, abre hacia abajo`)
await page.evaluate(() => { document.querySelector('.app').style.removeProperty('--sidebar-w-exp') })
...
const portaled = await page.evaluate(() => {
  ...
  if (!p) return null
  let n = p.parentElement, clipping = null
  while (n && n !== document.body) {
    const ov = getComputedStyle(n).overflow
    if (ov.includes('hidden') || ov.includes('auto') || ov.includes('scroll')) { clipping = n.className || n.tagName; break }
    n = n.parentElement
  }
  return { parentIsBody: p.parentElement === document.body, clipping }
})
log(portaled?.parentIsBody && !portaled.clipping,
...
log(layout.bodyOverflow <= 0, `regla 4 (sin scroll horizontal en la página): overflow ${layout.bodyOverflow}px`)
log(layout.mainOverflow <= 0, `regla 4 (sin scroll horizontal en #main): overflow ${layout.mainOverflow}px`)
...
log(fits.overflows <= 0 && fits.ellipsis === 'ellipsis',
...
log(!!modalVal, `elegir dentro del modal escribe el hidden input que lee openTask() ("${modalVal}")`)
...
log(modalIslands === 1 && modalInputs === 1,
