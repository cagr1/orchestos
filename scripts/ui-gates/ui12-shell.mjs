...
const projectRequest = (path, projectId) =>
...
try {
  ...
  const cold = await page.evaluate(() => {
    ...
    return {
      ...
      controls: ['#navSearchBtn', '#navCollapseBtn', '#rpToggle'].every((id) =>
  ...
  log(
    Boolean(
      cold.header && cold.header.x === 0 && cold.header.width === 1280 && cold.header.height === 44,
    ),
    'header cruza el viewport y mide 44px',
  )
  log(
    cold.wordmark?.replace(/\s/g, '') === 'OrchestOS' && cold.controls && !cold.status,
    'wordmark y controles del shell presentes, sin statusBadge',
  )
  ...
  log(
    await page.locator('#sidebar').evaluate((node) => getComputedStyle(node).width === '0px'),
    'navCollapseBtn oculta completamente el sidebar',
  )
  await page.locator('#navCollapseBtn').click()
  await page.waitForFunction(
  ...
  log(
    await page
      .locator('#sidebar')
      .evaluate((node) => Math.abs(node.getBoundingClientRect().width - 256) <= 1),
    'navCollapseBtn restaura el sidebar de 256px',
  )

  await page.locator('#shellModeDev').click()
  await page.locator('.dev-empty-logo img').waitFor({ state: 'visible' })
  log(
    (await page.locator('.dev-empty-logo img').isVisible()) &&
      (await page.locator('[data-workspace-project]').count()) === 0,
    'Dev vacío muestra logo y no abre un workspace',
  )
  ...
  log(
    Math.abs(emptyLogoBox.box.width - 96) <= 1 &&
      Math.abs(emptyLogoBox.box.height - 96) <= 1 &&
      Boolean(
        emptyLogoBox.image &&
          emptyLogoBox.image.width <= emptyLogoBox.box.width &&
          emptyLogoBox.image.height <= emptyLogoBox.box.height,
      ),
    'el logo de Dev vacío cabe en un cuadro de 96px',
  )
  ...
  const projectNamesFit = await projects
    .locator('.sidebar-project-name')
  ...
  log(
    Math.abs(folderMetrics.width - 16) <= 1 &&
      Math.abs(folderMetrics.height - 16) <= 1 &&
      Math.abs(folderMetrics.svgWidth - 16) <= 1 &&
      Math.abs(folderMetrics.svgHeight - 16) <= 1 &&
      folderMetrics.color === accent,
    'cada proyecto muestra una carpeta de 16px en color accent',
  )
  const sessionCount = await page.evaluate(
    async (id) =>
      (await (await fetch(`/api/chat/sessions?project=${encodeURIComponent(id)}`)).json()).length,
    firstId,
  )
  await page.waitForFunction(
    ({ id, count }) =>
      document.querySelector(`[data-project-id="${CSS.escape(id)}"] .sidebar-project-count`)
        ?.textContent === String(count),
    { id: firstId, count: sessionCount },
  )
  log(
    (await first.locator('.sidebar-project-count').innerText()) === String(sessionCount),
    'el chip de proyecto coincide con /api/chat/sessions',
  )
  ...
  const hiddenActions = await first.locator('[data-project-action]').evaluateAll((nodes) =>
    nodes.every((node) => {
  ...
  const hoverState = await first.locator('[data-project-action]').evaluateAll((nodes) => ({
    visible: nodes.every(
      (node) =>
    ...
    chipHidden:
      getComputedStyle(
        nodes[0].closest('.sidebar-project-row')?.querySelector('.sidebar-project-count'),
      ).display === 'none',
  }))
  const actionCenters = await first.locator('[data-project-action]').evaluateAll((nodes) =>
    nodes.map((node) => {
      const rect = node.getBoundingClientRect()
      return rect.top + rect.height / 2
    }),
  ...
  const aligned =
    Math.abs(chipCenter - rowCenter) <= 1 &&
    actionCenters.every((center) => Math.abs(center - rowCenter) <= 1)
  log(
    hiddenActions && hoverState.visible && hoverState.chipHidden && aligned,
    'acciones ocultas sin hover, visibles al pasar y centradas en la fila',
  )
  ...
  log(
    (await first.locator('.sidebar-agent-row').count()) === sessionCount &&
      (await page.evaluate(() => window.state.screen)) === beforeScreen,
    'clic en la fila solo expande sus agentes',
  )
  ...
  log(
    (await page.locator('[data-project-title]').innerText()).trim() === firstName,
    'Project settings abre desde el menú del proyecto',
  )
  log(
    (await page.locator('[data-project-tab]').count()) === 8,
    'Project settings muestra las ocho pestañas',
  )
  ...
  await page.locator('.project-settings-head').waitFor({ state: 'visible' })
  log(
    projectRequest('/api/tasks', firstId) && projectRequest('/api/runs', firstId),
    'Tasks y Runs usan el header del proyecto',
  )
  ...
  const newChatMetrics = await newChat.evaluate((node) => {
    ...
    return {
      ...
      iconCenter: icon ? icon.top + icon.height / 2 : NaN,
      labelCenter: label ? label.top + label.height / 2 : NaN,
    }
  })
  log(
    newChatMetrics.height <= 36 &&
      Math.abs(newChatMetrics.iconCenter - newChatMetrics.labelCenter) <= 1,
    'New chat mide hasta 36px y centra el + con el texto',
  )
  await page.locator('#shellModeDev').click()
  ...
  await page.waitForFunction(
    () => Math.abs(document.querySelector('#rightpanel').getBoundingClientRect().width - 384) <= 1,
  )
  log(
    await page
      .locator('#rightpanel')
      .evaluate((node) => Math.abs(node.getBoundingClientRect().width - 384) <= 1),
    'rpToggle abre el panel derecho de 384px',
  )
  await page.locator('#rpToggle').click()
  await page.waitForFunction(
  ...
  log(
    await page.locator('#rightpanel').evaluate((node) => getComputedStyle(node).width === '0px'),
    'rpToggle cierra el panel derecho',
  )
} catch (error) {
