...
const gitEnv = {
  ...
  GIT_AUTHOR_EMAIL: 's6-fixture@example.invalid',
  GIT_COMMITTER_NAME: 'S6 fixture',
  GIT_COMMITTER_EMAIL: 's6-fixture@example.invalid',
}
...
try {
  ...
  // catalog. It is outside S.6 and requires a real key; keep that unrelated network call from
  // creating console noise before the gate opens Plan through the sidebar.
  ...
  // the same-origin CSRF gate as a user action. Neither rejection may write a partial edge or
  // close the blocked card.
  ...
  const afterNegativeChecks = await page.evaluate(
