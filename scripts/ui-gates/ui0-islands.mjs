...
log(errors.length === 0, `sin errores de consola en carga normal (${errors.join(' | ') || 'ninguno'})`)
...
const varSurface = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--surface').trim())
const varBorder = await page.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--border').trim())
const hex = c => '#' + c.match(/\d+/g).slice(0,3).map(n => (+n).toString(16).padStart(2,'0')).join('')
log(hex(cardBg) === varSurface.toLowerCase(), `bg-card resuelve a --surface (${hex(cardBg)} vs ${varSurface})`)
log(hex(cardBorder) === varBorder.toLowerCase(), `border-border resuelve a --border (${hex(cardBorder)} vs ${varBorder})`)
...
const h1margin = await page.evaluate(() => {
  const h = document.createElement('h1'); h.textContent = 'x'; document.body.appendChild(h)
  const m = getComputedStyle(h).marginTop; h.remove(); return m
})
log(h1margin !== '0px', `preflight de Tailwind ausente: <h1> conserva su margen de UA (${h1margin})`)
...
log(en === 'Add task' && es === 'Agregar tarea', `t() repinta en vivo al cambiar idioma (en="${en}" → es="${es}")`)
log(beforeLang === afterLang && afterLang.includes('2'), `la isla se REPINTA, no se remonta: estado local sobrevive ("${beforeLang}" → "${afterLang}")`)
...
for (let i = 0; i < 5; i++) { await page.evaluate(() => window.App.rerender()); await page.waitForTimeout(120) }
await page.waitForTimeout(300)
const afterMany = await page.evaluate(() => document.querySelectorAll('[data-island="lang-probe"]').length)
const finalCount = await mountedAfter()
log(afterMany === 1, `sin duplicados tras 6 rerender() (contenedores: ${afterMany})`)
log(Number(finalCount) === 1, `sin leak de roots React tras 6 rerender() (montadas: ${finalCount}, antes: ${before}/${after})`)
...
await page.waitForTimeout(500)
const afterNav = await page.evaluate(() => document.querySelectorAll('[data-island="lang-probe"]').length)
log(afterNav === 1, `navegación entre pantallas no duplica ni pierde la isla (${afterNav})`)
log(errors.length === 0, `sin errores de consola con la isla activa (${errors.join(' | ') || 'ninguno'})`)
