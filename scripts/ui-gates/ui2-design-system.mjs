...
log(
  (await page.locator('[data-island="ui-kit-probe"]').count()) === 0,
...
log(
  btnReact.padding === btnVanilla.padding,
  `Button: padding igual al .btn vanilla (${btnReact.padding})`,
)
...
log(
  btnReact.bg === btnVanilla.bg,
  `Button variant=secondary: mismo fondo que .btn neutro (${btnReact.bg})`,
)
...
const variantColors = await page.evaluate(() =>
  Object.fromEntries(
    [...document.querySelectorAll('[data-variant]')].map((el) => [
      el.dataset.variant,
      getComputedStyle(el).backgroundColor + '|' + getComputedStyle(el).color,
    ]),
  ),
)
...
log(
  disabledClickable.disabled && disabledClickable.pointer === 'none',
  'Button: disabled no recibe clicks',
)
...
const tabVanilla = await page.evaluate(() => {
  const el = document.createElement('button')
  el.className = 'filter-tab active'
  el.textContent = 'x'
  document.body.appendChild(el)
  const cs = getComputedStyle(el)
  const v = {
...
log(
  tabReact.radius === tabVanilla.radius &&
    tabReact.padding === tabVanilla.padding &&
    tabReact.font === tabVanilla.font &&
    tabReact.bg === tabVanilla.bg,
...
log(
  Number(dialogStyle.z) === 300,
  `Dialog: z-index 300, sobre el scrim vanilla (200) y bajo los toasts (9999)`,
)
...
const hasTransition = await dialog.evaluate(
  (el) => getComputedStyle(el).transitionDuration !== '0s',
)
log(
  opacity === '1' && hasTransition,
  `Dialog: la transición de apertura existe de verdad (opacity ${opacity})`,
)
...
await page.evaluate(() => {
  eval("showToast('desde el vanilla', undefined)")
})
await page.waitForTimeout(400)
const globalHijacked = await page.evaluate(
  () =>
    document.querySelector('#orchestos-toaster')?.textContent?.includes('desde el vanilla') ??
    false,
)
log(globalHijacked, 'Toast: showToast() global redirigida — los 71 call sites vanilla no se tocan')

await page.evaluate(() => {
  eval("showToast('segundo aviso', 'error')")
})
await page.waitForTimeout(400)
...
log(
  stacked === 2,
  `Toast: dos avisos se APILAN en vez de superponerse como en vanilla (${stacked})`,
)
...
const toastGeo = await page.evaluate(() => {
  ...
  return {
    ...
    live: vp.getAttribute('aria-live') ?? vp.closest('[aria-live]')?.getAttribute('aria-live'),
  }
})
...
await page.evaluate(() => {
  eval("showToast('tras rerender')")
})
await page.waitForTimeout(400)
log(
  await page.evaluate(
    () =>
      document.querySelector('#orchestos-toaster')?.textContent?.includes('tras rerender') ?? false,
  ),
