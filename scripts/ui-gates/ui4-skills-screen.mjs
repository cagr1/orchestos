    ...
    window.state.skills = [
      {
        id: 'skill-alfa',
        name: 'Skill Alfa',
        description: 'primera skill sembrada',
        targets: ['claude', 'codex'],
      },
      {
        id: 'skill-beta',
        name: 'Skill Beta',
        description: 'segunda skill sembrada',
        targets: ['claude'],
      },
    ]
...
log(
  (await page.locator('[data-island="screen-skills"]').count()) === 1,
...
log(
  (await page.locator('.skills-pro-section').count()) === 1,
  'la sección de skills pro está presente',
)
...
log(
  proImportedDisabled.importadaDeshabilitada && proImportedDisabled.sinImportarHabilitada,
...
await page
  .locator('.skill-card')
  .first()
  .locator('button', { hasText: /Delet|Borrar|Elimin/i })
  .first()
  .click()
...
await page
  .locator('.skill-card-confirm:visible button', { hasText: /Cancel/i })
  .first()
  .click()
await page.waitForTimeout(400)
log(
  (await page.locator('.skill-card-confirm:visible').count()) === 0,
  'cancelar oculta la confirmación',
)
...
log(
  (await page.evaluate(
    () => document.querySelector('[data-island="screen-skills"]')?.dataset.gateMark,
  )) === 'vivo',
  'App.rerender() NO destruye el DOM de la pantalla (el nodo es el mismo)',
)
log(
  (await page.locator('[data-island="screen-skills"]').count()) === 1,
  'sin islas duplicadas tras el repintado',
)
...
await page.evaluate(() => {
  window.state.skills = [
    ...window.state.skills,
    {
      id: 'skill-gamma',
      name: 'Skill Gamma',
      description: 'agregada por el gate',
      targets: ['claude'],
    },
  ]
  window.App.rerender()
...
log(
  (await page.locator('.skill-card').count()) === 5,
...
for (const [status, selector, label] of [
  ...
  await page.evaluate((s) => {
    window.state.skillsStatus = s
    window.App.rerender()
  }, status)
  await page.waitForTimeout(500)
  log(
    (await page.locator(selector).count()) === 1,
    `el estado de ${label} se renderiza igual que en vanilla`,
  )
}
await page.evaluate(() => {
  window.state.skillsStatus = 'ok'
...
await page.evaluate(() => {
  localStorage.setItem('orchestos-mode', 'advanced')
  window.App.go('runs')
})
...
const vanillaBadge = await page.evaluate(() => {
  ...
  el.className = 'badge blue square'
  el.textContent = 'x'
...
log(
  reactH1 && vanillaH1 && reactH1.font === vanillaH1.font && reactH1.color === vanillaH1.color,
  `paridad del título contra Runs (React ${reactH1?.font} vs vanilla ${vanillaH1?.font})`,
)
log(
  reactBtn && vanillaBtn && reactBtn.pad === vanillaBtn.pad && reactBtn.font === vanillaBtn.font,
  `paridad de los botones de cabecera (React ${reactBtn?.pad} vs vanilla ${vanillaBtn?.pad})`,
)
log(
  reactBadge &&
    vanillaBadge &&
    reactBadge.font === vanillaBadge.font &&
    reactBadge.radius === vanillaBadge.radius,
