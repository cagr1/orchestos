...
try {
  ...
  await page.waitForFunction(() =>
    document.querySelector('#rpTabTerminal')?.classList.contains('active'),
  )
  log(
    await page.locator('#rpTabTerminal').evaluate((node) => node.classList.contains('active')),
    'Terminal cambia la herramienta activa',
  )
  const projects = page.locator('[data-project-id]')
  if ((await projects.count()) > 1) {
    await projects.nth(1).click()
    log(
      await page.locator('.app').evaluate((node) => node.dataset.rightpanel === 'expanded'),
      'expandir otro proyecto conserva el inspector abierto',
    )
  } else {
  ...
  log(
    ...
    'rpClose cierra el inspector',
  )
  log(
    !(await page.locator('#rpToggle').evaluate((node) => node.classList.contains('active'))),
    'el toggle queda inactivo al cerrar',
  )
  await page.locator('#rpToggle').click()
  await page.waitForFunction(() =>
    document.querySelector('#rpTabTerminal')?.classList.contains('active'),
  ...
  log(
    await page.locator('#rpTabTerminal').evaluate((node) => node.classList.contains('active')),
    'reabre la última herramienta usada',
  )
} catch (error) {
