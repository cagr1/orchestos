...
function buildCutoffs(now: Date, policy: RetentionPolicy): { runSteps: string; chatTaskBarEvents: string } {
  return {
...
export function previewEphemeralData(
  ...
  return {
    runSteps: db.query<{ count: number }, string>(
      'SELECT COUNT(*) AS count FROM run_steps WHERE created_at < ?'
    ).get(cutoffs.runSteps)?.count ?? 0,
    chatTaskBarEvents: db.query<{ count: number }, string>(
...
export function purgeEphemeralData(
  ...
  const purge = db.transaction(() => {
    const runSteps = db.run('DELETE FROM run_steps WHERE created_at < ?', [cutoffs.runSteps]).changes
    const chatTaskBarEvents = db.run(
      'DELETE FROM chat_task_bar_events WHERE created_at < ?',
