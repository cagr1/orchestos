...
export async function runTask(opts: HarnessOpts): Promise<TaskResult> {
  ...
  try {
    ...
    // -- build prompt ----------------------------------------------------------
    const { system, userContent } = buildPrompt(effectiveTask, contextText, projectRoot, constitutionBlock)
