...
import {
  APPLY_THRESHOLD,
  type InsertInstinctDef,
  type InstinctDef,
  type InstinctSource,
  recalculateVerified,
  type UpdateInstinctDef,
  validateInsert,
  validateInstinct,
} from './schema.ts'
...
export function getInstinct(id: string): InstinctDef | null {
  const row = db.query<InstinctDefRow, [string]>('SELECT * FROM instincts WHERE id = ?').get(id)
...
export function updateConfidence(id: string, newConfidence: number): boolean {
  ...
  db.run('UPDATE instincts SET confidence = ?, verified = ? WHERE id = ?', [
    newConfidence,
    verified,
    id,
  ])
...
export function updateInstinct(id: string, data: UpdateInstinctDef): boolean {
  ...
  if (data.confidence !== undefined) {
    ...
    const verified = recalculateVerified(data.confidence, data.verified ?? existing.verified)
      ? 1
      : 0
    sets.push('verified = ?')
  ...
  params.push(id)
  db.run(`UPDATE instincts SET ${sets.join(', ')} WHERE id = ?`, params)
  return true
}
...
export function approveInstinct(id: string): boolean {
  ...
  db.run('UPDATE instincts SET verified = 1, confidence = ? WHERE id = ?', [newConfidence, id])
  return true
}
