...
// Los perfiles de fixtures no prueban instalaciones locales de toolchains. Evita
// ejecutar bunx/cargo/go en cada fixture; el test dedicado de Bun cubre el dato real.
const noToolchain: (name: string, args: string[]) => string | null = () => null
const profile = (root: string) => buildRoadmapProfile(root, { probe: noToolchain })
...
describe('M.3 — buildRoadmapProfile: disciplinas', () => {
  it('proyecto vacío: nada aplica, sin inventar un pass falso', async () => {
    ...
    const byDiscipline = Object.fromEntries(result.disciplines.map((d) => [d.discipline, d.state]))
  ...
  it('detecta backend por framework conocido en package.json', async () => {
    ...
    const result = await profile(root)
    const backend = result.disciplines.find((d) => d.discipline === 'backend')!
  ...
  it('detecta frontend por archivos .html', async () => {
    ...
    const result = await profile(root)
    expect(result.disciplines.find((d) => d.discipline === 'frontend')!.state).toBe('detected')
  })
  ...
  it('detecta qa-seguridad contando archivos *.test.ts reales', async () => {
    ...
    const result = await profile(root)
    const qa = result.disciplines.find((d) => d.discipline === 'qa-seguridad')!
  ...
  it('M.5: detecta qa-seguridad con convención Go (_test.go), no solo *.test.ts', async () => {
    ...
    const result = await profile(root)
    expect(result.disciplines.find((d) => d.discipline === 'qa-seguridad')!.state).toBe('detected')
  })
  ...
  it('detecta devops por workflows de CI', async () => {
    ...
    const result = await profile(root)
    expect(result.disciplines.find((d) => d.discipline === 'devops')!.state).toBe('detected')
    expect(result.ci.state).toBe('detected')
    expect(result.ci.evidence).toContain('ci.yml')
  })
})
...
describe('M.3 — buildRoadmapProfile: infraestructura', () => {
  it('CI missing cuando no existe .github/workflows', async () => {
    ...
    writePkg(root)
    const result = await profile(root)
    expect(result.ci.state).toBe('missing')
  })
  ...
  it('Docker detected con Dockerfile presente', async () => {
    ...
    const result = await profile(root)
    expect(result.docker.state).toBe('detected')
  })
  ...
  it('Docker not-applicable sin Dockerfile ni docker-compose', async () => {
    ...
    writePkg(root)
    const result = await profile(root)
    expect(result.docker.state).toBe('not-applicable')
  })
  ...
  it('database detected por dependencia conocida (prisma)', async () => {
    ...
    const result = await profile(root)
    expect(result.database.state).toBe('detected')
    expect(result.database.evidence).toContain('prisma')
  })
  ...
  it('instructionFiles: cada archivo se marca detected/missing según exista', async () => {
    ...
    const result = await profile(root)
    expect(result.instructionFiles['AGENTS.md']).toBe('detected')
    expect(result.instructionFiles['CLAUDE.md']).toBe('missing')
    expect(result.instructionFiles['CONTEXT.md']).toBe('missing')
    expect(result.instructionFiles['PLAN.md']).toBe('missing')
  })
})
...
describe('M.3 — perfiles de lenguaje (eje secundario, contra docs/roadmaps/languages/)', () => {
  it('known cuando existe el doc del lenguaje, incluso con sufijo distinto (typescript-bun.md)', async () => {
    ...
    const result = await profile(root)
    const ts = result.languageProfiles.find((l) => l.language === 'TypeScript')!
  ...
  it('missing cuando no hay doc de lenguaje todavía (ni local ni centralizado) — nunca inventa contenido', async () => {
    ...
    const result = await profile(root)
    const cs = result.languageProfiles.find((l) => l.language === 'C#')!
  ...
  it('proyecto políglota: registra cada lenguaje detectado, no reduce a uno primario', async () => {
    ...
    const result = await profile(root)
    const langs = result.languageProfiles.map((l) => l.language).sort()
...
describe('M.3 — dogfooding contra el propio repo de OrchestOS', () => {
  it('detecta backend/frontend/qa-seguridad/devops reales de este repo', async () => {
    const result = await profile(process.cwd())
    const byDiscipline = Object.fromEntries(result.disciplines.map((d) => [d.discipline, d.state]))
...
describe('M.3 — renderProjectProfileMarkdown', () => {
  it('incluye estado explícito por hallazgo, nunca texto suelto sin estado', async () => {
    ...
    writePkg(root, { dependencies: { express: '^4.0.0' } })
    const result = await profile(root)
    const md = renderProjectProfileMarkdown(result)
