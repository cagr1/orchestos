...
import { getRun, insertRun } from '../db/runs.ts'
...
describe('context_warnings_json — migration', () => {
  it('column exists in runs table after migration', () => {
    const cols = db
      .query<{ name: string }, string>('PRAGMA table_info(runs)')
      .all('runs')
      .map((r) => r.name)
...
describe('context_warnings_json — insertRun / getRun', () => {
  ...
  it('stores JSON string for a single warning', () => {
    const warnings = [
      { code: 'context_warning', severity: 'warning', message: 'Context low: 30% remaining' },
    ]
    ...
    const warnings = [
      {
        ...
        severity: 'critical',
        message: 'Context critically low: 18% remaining',
      },
    ...
    const warnings = [
      {
        code: 'loop_detected',
        severity: 'warning',
        message: "Possible loop: 'read' called 3+ times consecutively",
      },
    ]
    const id = insertRun({ ...baseRun(), context_warnings_json: JSON.stringify(warnings) })
    const r = getRun(id)!
