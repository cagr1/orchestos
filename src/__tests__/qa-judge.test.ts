...
import { RoleUnassignedError, resolveRole } from '../config/load.ts'
import type { OrcheConfig } from '../config/schema.ts'
import { runQA } from '../run/qa.ts'

describe('reviewer role assignment', () => {
  it('requires an explicit reviewer role', () => {
    const cfg: OrcheConfig = { config_version: 1, roles: {}, models: {} }
    expect(() => resolveRole(cfg, 'reviewer')).toThrow(RoleUnassignedError)
  })
  it('uses the configured reviewer model', () => {
    const cfg: OrcheConfig = {
      config_version: 1,
      roles: { reviewer: { agent: 'api', provider: 'openrouter', model: 'review-model' } },
      models: {},
    }
    expect(resolveRole(cfg, 'reviewer').model).toBe('review-model')
  })
})
