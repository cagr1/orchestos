...
describe('parseAffordableTokens', () => {
  it('extracts the affordable token count from a real OpenRouter 402 body', () => {
    const body =
      '{"error":{"message":"This request requires more credits, or fewer max_tokens. You requested up to 128000 tokens, but can only afford 118057. To increase, visit https://openrouter.ai/settings/credits and add more credits","code":402}}'
    expect(parseAffordableTokens(body)).toBe(118057)
  })

  it('returns null for an unrelated error body', () => {
    expect(
      parseAffordableTokens('{"error":{"message":"Rate limit exceeded","code":429}}'),
    ).toBeNull()
  })
