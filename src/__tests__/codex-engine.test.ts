...
describe('G.4.2b — codexEngine (codex subprocess)', () => {
  ...
  it('requests reasoning summaries on task and chat invocations', () => {
  ...
  it('happy path: JSONL con turn.completed → outcome con costo computado vía calcCost()', async () => {
    ...
    const ctx = buildCtx(wt, baseTask({ engine: 'codex' }))
    ctx.cliEffort = 'medium'
    ...
    expect(outcome.costByIteration[0]!.args).toContain('model_reasoning_effort=medium')
