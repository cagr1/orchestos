...
interface CliStatus {
  id: 'claude' | 'codex' | 'opencode' | 'deepseek' | 'gemini' | 'kimi' | 'glm'; label: string; binary: string; icon: string; installed: boolean; available: boolean; observedAt: string | null
...
export function SessionStatusBar() {
  ...
  return <section className="session-statusbar-inner" aria-label={t('session.status.label')}>
    ...
    <div className="session-statusbar-clis">{installedClis.map((cli) => <CliStatusItem key={cli.id} cli={cli} t={t} />)}</div>
    {installedClis.length === 0 && <span className="session-statusbar-empty">{t('session.status.none')}</span>}
  </section>
}
