...
describe('G.4 — POST /api/tasks acepta engine', () => {
  ...
  it('engine="codex" persiste el effort que el usuario elige', async () => {
    const res = await route(
      req('POST', '/api/tasks', {
        id: 'codex-task',
    ...
    expect(readFileSync(join(tmpDir, 'tasks.yaml'), 'utf-8')).toContain('cli_effort: turbo')
  })
