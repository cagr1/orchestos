...
export function isPrivateIP(ip: string): boolean {
  ...
  if (v6 >> 32n === 0xffffn)
    return isPrivateIP(
      `${Number((v6 >> 24n) & 255n)}.${Number((v6 >> 16n) & 255n)}.${Number((v6 >> 8n) & 255n)}.${Number(v6 & 255n)}`,
    )
  return (
    v6 === 0n ||
    v6 === 1n ||
    ipv6InRange(v6, 0xfc000000000000000000000000000000n, 7) ||
...
export async function checkSsrSafe(parsed: URL, lookupFn?: LookupFn): Promise<string | null> {
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:')
    return `[SSRF blocked: unsupported URL scheme — ${parsed.protocol}]`
  ...
  if (parsed.port && parsed.port !== '80' && parsed.port !== '443')
    return `[SSRF blocked: non-standard port ${parsed.port} is not allowed]`
