...
/*
 * `react: true` le dice a `App.rerender()` que NO repinte el DOM de esta pantalla: escribe el
 * contenedor una sola vez y despues solo avisa que el estado cambio (ver la nota larga en
 * `rerender()`). `wire()` queda vacio a proposito: los handlers son del componente React.
 */
/**
 * SKILLS — migrada a React en UI.4 (Mes 30). El componente vive en
 * `public-src/islands/screens/SkillsScreen.tsx`.
 *
 * `react: true` le dice a `App.rerender()` que NO repinte el DOM de esta pantalla (ver la
 * nota larga en `rerender()`): sin eso la isla se remontaria en cada poll de 30s.
 *
 * El render y el wire viejos NO quedan comentados aca. La primera pasada los dejo entre
 * `/* ... *​/` "por si acaso" — 260 lineas de codigo muerto que parece vigente, que es
 * justo lo que UI.5 va a tener que limpiar y lo que vuelve intocable un archivo. El
 * historial de git ya los conserva; para verlos: `git show HEAD~1 -- <este archivo>`.
 */
SCREENS.skills = {
  react: true,
  render() {
