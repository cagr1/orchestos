...
function processTextNode(textNode, re, idx) {
  ...
  for (const match of matches) {
    ...
    if (match.meta.type === 'task') {
      ...
      span.setAttribute('tabindex', '0')
      span.title =
        match.meta.name && match.meta.name !== match.meta.id
...
function renderMarkdown(raw, st) {
  ...
  ;(function sanitize(node) {
    for (const child of [...node.children]) {
      if (!SAFE_MD_TAGS.has(child.tagName.toLowerCase())) {
        child.remove()
        continue
      }
...
function renderStepsCard(st, taskId) {
  ...
  const rows = entry.steps
    .map((s) => {
      ...
      const label =
        s.type === 'tool_use' ? `${t('chat.steps.toolUse')} ${esc(s.label)}` : esc(s.label)
...
SCREENS.chat = {
  render(st) {
      ...
      history.length === 0
        ...
        : history
            .map((m, i) => {
              ...
              const ocrTag =
                m.role === 'assistant' && m.ocrUsed && m.ocrUsed.length
    ...
    const localWarnDismissed =
      typeof sessionStorage !== 'undefined' && sessionStorage.getItem('ollama-warn-shown')
    const localWarnBanner =
      isLocalModel && !localWarnDismissed
    ...
    const attachChips =
      chatFiles.length > 0
        ? `<div class="chat-attach-chips">${chatFiles
            .map(
              (f) => `
  ...
  wire(root, st) {
    ...
    root.querySelectorAll('[data-chat-copy]').forEach((btn) =>
    ...
    root.querySelectorAll('[data-attach-kind]').forEach((btn) =>
      btn.addEventListener('click', () => {
    ...
    root.querySelectorAll('[data-act="chat-file-remove"]').forEach((btn) =>
      btn.addEventListener('click', () => {
    ...
    root
      .querySelector('[data-modelfx-panel] [data-combo-search]')
...
SCREENS.tasks = {
  diagnoseDetail(d, st) {
    ...
    const confKey =
      d.confidence === 'high'
        ? 'tasks.diagnose.conf.high'
        : d.confidence === 'medium'
          ? 'tasks.diagnose.conf.med'
  ...
  render(st) {
    ...
    const rows = tasks
      .map((task) => {
        const MAX = 72
        const desc =
          task.description.length > MAX
            ? task.description.slice(0, MAX - 1) + '…'
            : task.description
  ...
  wire(root, st) {
    ...
    if (initBtn)
      initBtn.addEventListener('click', async () => {
        const ok = await Modal.confirm(
          t('tasks.init.confirm.title'),
          t('tasks.init.confirm.body'),
          t('tasks.init.btn'),
        )
        if (!ok) return
        const btn = initBtn
    ...
    advPanel?.addEventListener('toggle', () => {
      st.draftAdvancedOpen = advPanel.open
    })
    ...
    root.querySelectorAll('[data-suggest-file]').forEach((btn) =>
      btn.addEventListener('click', () => {
    ...
    root.querySelector('[data-act="draft-confirm"]')?.addEventListener('click', async () => {
      ...
      const modelId =
        root.querySelector('#draft-model')?.value?.trim() || 'deepseek/deepseek-v4-flash'
      const engine = root.querySelector('#draft-engine')?.value || ''
      ...
      if (!id || !desc) {
        if (msg2) {
          msg2.textContent = 'ID y descripción son obligatorios.'
          msg2.style.color = 'var(--error)'
      ...
      try {
        ...
        if (!createRes.ok) {
          ...
          if (msg2) {
            msg2.textContent = e.error || 'Error al crear la tarea.'
            msg2.style.color = 'var(--error)'
            msg2.style.display = 'flex'
          }
          return
        ...
        if (msg2) {
          msg2.textContent = 'Error de conexión.'
          msg2.style.color = 'var(--error)'
          msg2.style.display = 'flex'
        }
      } finally {
    ...
    root.querySelectorAll('[data-task]').forEach((tr) =>
    ...
    // A3 — diagnose click
    root.querySelectorAll('[data-diag]').forEach((el) =>
      el.addEventListener('click', async (e) => {
        ...
        try {
          ...
          st.diagnoseCache[id] = {
            error: err && err.message ? err.message : t('tasks.diagnose.err'),
          }
        }
        st.openDiagnose = id
        App.rerender()
    ...
    // A4 — retry button
    root.querySelectorAll('[data-act="retry"]').forEach((btn) =>
      btn.addEventListener('click', async (e) => {
        e.stopPropagation()
    ...
    root.querySelectorAll('[data-act="make-habit"]').forEach((btn) =>
      btn.addEventListener('click', async (e) => {
        e.stopPropagation()
...
SCREENS.memory = {
  render(st) {
    ...
    const conflictsPanel =
      conflicts.length === 0
        ? ''
        : `<div class="proj-helper warn">
      <strong>${t('memory.conflicts.title', conflicts.length)}</strong>
      <div style="margin-top:6px;display:flex;flex-direction:column;gap:6px">${conflicts
        .map(
          (c) =>
    ...
    const counts = (sc) =>
      sc === 'all' ? memory.length : memory.filter((m) => m.scope === sc).length
    ...
    if (st.memoryStatus === 'loading') {
      ...
      const selectAllRow = rows.length
        ? `<label class="mem-select-all" style="display:flex;align-items:center;gap:8px;font-size:12px;color:var(--text-muted);margin-bottom:8px;cursor:pointer">
      ...
      right = rows.length
        ...
        : emptyState(
            ICON.memory,
            t('memory.no.title'),
            q ? t('memory.no.body', esc(q)) : t('memory.empty.body'),
          )
    }
  ...
  wire(root, st) {
    wireBulkSelect(
      ...
      () => App.fetchMemory(st.memQuery || undefined),
      t('bulk.resource.memory'),
    )
    let _searchTimer
    ...
    root.querySelectorAll('[data-sc]').forEach((s) =>
      s.addEventListener('click', () => {
        st.memScope = s.dataset.sc
        App.rerender()
      }),
    )
    root.querySelectorAll('[data-resolve-conflict]').forEach((b) =>
      b.addEventListener('click', async () => {
        const id = b.dataset.resolveConflict
        b.disabled = true
        try {
          const res = await fetch(`/api/memory/conflicts/${encodeURIComponent(id)}/resolve`, {
            method: 'POST',
          })
    ...
    root.querySelectorAll('[data-topic]').forEach((c) => {
      c.addEventListener('click', () => {
        ...
        if (note)
          note.textContent = c.classList.contains('exp') ? t('common.collapse') : t('common.expand')
      })
    ...
    root.querySelectorAll('[data-mem-act="delete"]').forEach((btn) =>
      btn.addEventListener('click', async (e) => {
        e.stopPropagation()
        const ok = await Modal.confirm(
