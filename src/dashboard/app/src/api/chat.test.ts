...
describe('chat API mapping', () => {
  test('maps persisted session and message rows to the prototype types', () => {
    const thread = mapSessionToThread({
      id: 'session-1', projectId: null, agent: 'api', mode: 'chat', title: 'Real chat',
      createdAt: '2026-09-21T12:00:00.000Z', updatedAt: '2026-09-21T12:01:00.000Z',
    }, [{
      id: 1, sessionId: 'session-1', role: 'assistant', content: 'Persisted answer', model: 'model-x',
      taskId: null, ocrUsed: [], taskHeld: false, existingFiles: [], createdAt: '2026-09-21T12:01:00.000Z',
    }]);
    expect(thread).toMatchObject({ id: 'session-1', title: 'Real chat', agent: 'api' });
    expect(thread.messages[0]).toMatchObject({ id: '1', content: 'Persisted answer', model: 'model-x' });
  });
  ...
  test('sends the same session/message controls as the legacy composer', async () => {
    ...
    try {
      await sendMessage({
        sessionId: 'session-1', message: 'hello', agent: 'api', model: 'provider/model', effort: 'High',
    ...
    expect(body).toMatchObject({ sessionId: 'session-1', history: [], message: 'hello', model: 'provider/model', effort: 'high', fileIds: ['file-1'] });
