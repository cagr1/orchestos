...
export function mapSessionToAgent(session: SessionRow, status: TurnStatus = { kind: 'none' }): AgentSession {
  return {
    id: session.id,
    name: session.title,
    model: session.agent,
    ...
    filesCount: 0,
    lastLog: status.kind === 'failed' ? 'The last turn failed.' : status.kind === 'interrupted' ? 'The last turn was interrupted.' : undefined,
    branch: '',
  };
}

async function loadAgent(session: SessionRow): Promise<AgentSession> {
  const status = await request<TurnStatus>(`/api/chat/sessions/${encodeURIComponent(session.id)}/turn-status`)
...
export async function listProjects(): Promise<ProjectItem[]> {
  ...
  return Promise.all(rows.map(async (row) => {
    const sessions = await request<SessionRow[]>(`/api/chat/sessions?project=${encodeURIComponent(row.id)}`);
