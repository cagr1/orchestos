...
const DETECTORS: Array<{ kind: SecretKind; pattern: RegExp }> = [
  ...
  },
  {
    kind: 'provider-key',
    pattern:
      /\b(?:sk-or-v1-[A-Za-z0-9_-]{20,}|sk-ant-api\d{2}-[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9_-]{24,})\b/g,
  },
  {
    kind: 'github-token',
  ...
  { kind: 'bearer-token', pattern: /\bBearer\s+[A-Za-z0-9._~+/=-]{20,}/gi },
  {
    ...
    pattern:
      /\b(?:api[_-]?key|token|secret|password|private[_-]?key|authorization)\s*[:=]\s*["']?([A-Za-z0-9_./+=-]{16,})["']?/gi,
  },
]
...
export function findSecrets(text: string): SecretFinding[] {
  ...
  for (const detector of DETECTORS) {
    ...
    for (const match of text.matchAll(detector.pattern)) {
      ...
      if (
        !isPlaceholder(credential) &&
        !(detector.kind === 'credential-assignment' && hasSpecificSecretKind(credential))
      ) {
