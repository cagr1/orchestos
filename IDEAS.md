...
### 21. Auto-selección semántica de skill + skills de diseño (dos gaps, uno depende del otro)

**Origen**: prueba real de Carlos (2026-07-06) — pidió una landing comercial "usando skills de
diseño"; el resultado no fue el esperado. Diagnóstico con código real destapó DOS gaps
separados:

**Gap A — falta el motor de auto-selección de skill (elegido por Carlos como prioridad).**
Hoy las skills solo se aplican si la tarea declara `task.skill` explícitamente:
- [`skill-route.ts`](src/run/middlewares/skill-route.ts) lee únicamente `ctx.task.skill`.
- [`/api/natural`](src/dashboard/handlers/project.ts) (creador de tareas en lenguaje natural)
  **no setea `skill`** — solo devuelve `id`/`description`/`output`/`executor`.
- `when_to_use` (los triggers en el `SkillDef`) es **decorativo para routing** — solo se
  renderiza al *exportar* una skill ([`targets/_shared.ts`](src/skills/targets/_shared.ts)),
  nunca se usa para matchear tarea→skill.
El motor a construir: un clasificador que mire la tarea y elija la skill correcta por
`when_to_use`, sin que el usuario la ate a mano — "invisible para el usuario". **Es el mismo
patrón que [[#4]] (clasificador semántico de `clarify`) y que el B.1.b del Mes 18** (detección
semántica de intención): los tres son "mirar lenguaje natural y elegir la acción/skill
correcta". Candidato fuerte a un primitivo reusable de clasificación semántica en vez de tres
implementaciones separadas.

**Matiz de arquitectura (decisión Carlos, 2026-07-06)**: cuando el clasificador encuentre
**más de una** skill candidata del mismo dominio (ej. varias de diseño: frontend-design,
taste, ui-ux…), NO elige una a ciegas — **presenta las opciones al usuario final para que él
decida** (mismo principio "sugerir, nunca auto-ejecutar" del Mes 18: el composer muestra los
candidatos con su descripción, el usuario elige uno o ninguno). Auto-selección silenciosa solo
cuando hay UN candidato claro; ambigüedad → elección humana. Esto además resuelve el empate
sin necesidad de un ranking sofisticado.

**Gap B — no existe ninguna skill de diseño (dependencia de A para el caso de Carlos).**
`skills/` + `skills/pro/` son 21 skills, todas de ingeniería (tdd, diagnose, qa, refactor…).
Las "skills de diseño" que Carlos cataloga en `~/Documents/projects/memories/skills/design-skills.md`
son **punteros** a plugins/MCP externos de **Claude Code** (Impeccable, UI-UX-Pro-Max, Taste,
Open Design MCP, Google design.md) — el contenido real vive en esos repos de GitHub. OrchestOS
no carga plugins de Claude Code ni MCP (eso es [[#10]], cliente MCP, backlog largo). Traerlas
= seguir el puntero y **destilar el conocimiento como `SkillDef` YAML nativo**.
**Progreso Gap B (2026-07-06) — ✅ completo, 4 skills de diseño nativas escritas y verificadas
en vivo contra el dashboard real:**
- `skills/frontend-design.yaml` — destilada de Impeccable (Apache 2.0): craft rules, bans
  absolutos, AI-slop test.
- `skills/ux-guidelines.yaml` — destilada de UI-UX-Pro-Max (MIT): checklist medible por
  prioridad (a11y, touch, performance, forms, nav).
- `skills/design-brief-inference.yaml` — destilada de taste-skill (MIT) + DESIGN.md philosophy
  (Apache 2.0): cómo leer el brief y declarar dirección concreta ANTES de picar código — el
  gap real que causó la prueba fallida de Carlos.
- `skills/design-tokens.yaml` — destilada de UI-UX-Pro-Max: arquitectura de tokens de 3 capas +
  spec de estados de componente.
Descartado deliberadamente (documentado para que no sea sorpresa): banner/slides/logo/CIP/
brandkit/imagegen-* (dependen de generación de imágenes vía Gemini, fuera del rol de OrchestOS);
minimalist/brutalist/soft/redesign-skill de Taste (ya cubiertas conceptualmente por los dials de
`design-brief-inference`, traerlas aparte sería ruido repetido); `brand`/`ui-styling` de
UI-UX-Pro-Max (delgadas, mayormente comandos de instalación, sin conocimiento propio).

**Progreso Gap A (2026-07-06) — doc de diseño escrito, pendiente de implementar:**
[`docs/semantic-skill-selection-design.md`](../docs/semantic-skill-selection-design.md) — un LLM
call barato con la lista de `when_to_use` de todas las skills instaladas → 0/1/varios candidatos;
0 = nada, 1 = auto-asignado en el draft (revisable), varios = **el composer los presenta y el
usuario final elige** (matiz de Carlos, 2026-07-06 — mismo principio "sugerir, nunca
auto-ejecutar" del Mes 18). Deja registrado que comparte forma con IDEAS #4 (`clarify`) y el
clasificador de intención del Mes 18 — candidato a un primitivo `classifyAgainstOptions()`
compartido cuando se implemente el segundo de los tres, sin forzar el orden.

**Nota de dependencia — ya resuelta**: el mínimo para "landing con criterio de diseño que se
aplica sola" era A + ≥1 skill de diseño (Gap B). Gap B ya está completo (4 skills); falta
implementar A (el doc de diseño) para que el caso de Carlos se resuelva de punta a punta.

**Esfuerzo**: A = medio (un LLM call barato que recibe la task + la lista de `when_to_use` de
las skills disponibles y devuelve la mejor o ninguna; fail-safe a ninguna, mismo espíritu que
IDEAS #4 / Mes 18). B = medio por skill (autoría de contenido, sin motor). **Requiere doc de
diseño antes de código** (patrón del repo: `external-executor-design.md`,
`chat-task-detection-design.md`).
